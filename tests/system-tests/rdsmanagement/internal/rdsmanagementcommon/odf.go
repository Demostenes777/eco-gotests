...
var (
	...
	storageClassName = rdsmanagementparams.StorageClassName
...
func VerifyODFNamespaceExists(ctx SpecContext) {
	...
	Expect(err).ToNot(HaveOccurred(), fmt.Sprintf("Failed to pull %q namespace", rdsmanagementparams.ODFNamespace))
} // func VerifyODFNamespaceExists (ctx SpecContext)
...
func VerifyODFDeployment(ctx SpecContext) {
	for _, operatorPod := range odfOperatorDeployments {
		glog.V(rdsmanagementparams.RdsManagementLogLevel).Infof("Confirm that odf %s pod was deployed and running in %s namespace",
			operatorPod, rdsmanagementparams.ODFNamespace)

		odfPods, err := pod.ListByNamePattern(APIClient, operatorPod, rdsmanagementparams.ODFNamespace)
		Expect(err).ToNot(HaveOccurred(), fmt.Sprintf("No %s pods were found in %s namespace; %v",
			operatorPod, rdsmanagementparams.ODFNamespace, err))
		...
		if err != nil {
			...
			glog.Fatalf("%s pod in %s namespace in a bad state: %s",
				odfPodName, rdsmanagementparams.ODFNamespace, odfPodLog)
		}
	}
	...
	for _, operatorDeployment := range odfOperatorDeployments {
		glog.V(rdsmanagementparams.RdsManagementLogLevel).Infof("Confirm that %s deployment is running in %s namespace",
			operatorDeployment, rdsmanagementparams.ODFNamespace)

		odfDeployment, err := deployment.Pull(APIClient, operatorDeployment, rdsmanagementparams.ODFNamespace)
		Expect(err).ToNot(HaveOccurred(), fmt.Sprintf("%s deployment not found in %s namespace; %v",
			operatorDeployment, rdsmanagementparams.ODFNamespace, err))
...
func VerifyODFConfig(ctx SpecContext) {
	glog.V(rdsmanagementparams.RdsManagementLogLevel).Infof("Enable odf-console")
	...
	glog.V(rdsmanagementparams.RdsManagementLogLevel).Infof("Start to configure ODF StorageCluster")
	...
	storageclusterObj, err := storage.NewStorageClusterBuilder(APIClient, storageclusterName, rdsmanagementparams.ODFNamespace).
	...
	Expect(err).ToNot(HaveOccurred(), fmt.Sprintf("Failed to create storageCluster %s instance in %s namespace; "+
		"%v", storageclusterName, rdsmanagementparams.ODFNamespace, err))
...
func VerifyODFSuite() {
	Describe(
		"ODF validation",
		Label(rdsmanagementparams.LabelRdsManagementOperators), func() {
			It(fmt.Sprintf("Verifies %s namespace exists", rdsmanagementparams.ODFNamespace),
